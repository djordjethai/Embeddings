...
def import_with_logging():
    ...
    try:
        ...
        logger.debug("Successfully imported prepare_embeddings, do_embeddings from myfunc.embeddings")
    except ImportError as e:
        logger.error(f"Failed to import prepare_embeddings, do_embeddings from myfunc.embeddings: {e}")

    try:
        from myfunc.retrievers import PineconeUtility
        logger.debug("Successfully imported PineconeUtility from myfunc.retrievers")
    except ImportError as e:
        logger.error(f"Failed to import PineconeUtility from myfunc.retrievers: {e}")

    try:
        from myfunc.various_tools import main_scraper
        logger.debug("Successfully imported main_scraper from myfunc.various_tools")
    except ImportError as e:
        logger.error(f"Failed to import main_scraper from myfunc.various_tools: {e}")
